(async function(){
  ...
  try{
    ...
    const response=await fetch(`${url}/rest/v1/products?select=id,name,description,measurements,price,active,sort_order,category_id,category:product_categories(id,name,active),image_url,image_path,image_url_2,image_path_2,image_url_3,image_path_3,image_url_4,image_path_4,video_url,video_path&active=eq.true&order=sort_order.asc,created_at.asc`,{headers});
