...
const mediaDeleteState={1:false,2:false,3:false,4:false,video:false};
...
async function loadData(){
  ...
  try{
    [products,categories]=await Promise.all([
      api("/rest/v1/products?select=id,name,description,measurements,price,active,sort_order,category_id,image_url,image_path,image_url_2,image_path_2,image_url_3,image_path_3,image_url_4,image_path_4,video_url,video_path&order=sort_order.asc,created_at.asc"),
...
window.editProduct=id=>{const p=products.find(x=>x.id===id);if(!p)return;Object.keys(mediaDeleteState).forEach(k=>mediaDeleteState[k]=false);document.querySelector("#productDialogTitle").textContent="Editar produto";document.querySelector("#productId").value=p.id;document.querySelector("#productName").value=p.name;document.querySelector("#productDescription").value=p.description||"";document.querySelector("#productMeasurements").value=p.measurements||"";document.querySelector("#productPrice").value=p.price;document.querySelector("#productActive").value=String(p.active);populateCategorySelect(p.category_id);document.querySelector("#productImage1").value="";document.querySelector("#productImage2").value="";document.querySelector("#productImage3").value="";document.querySelector("#productImage4").value="";document.querySelector("#productVideo").value="";document.querySelector("#productPreview1").innerHTML=p.image_url?`<img src="${p.image_url}" alt="">`:"Nenhuma imagem";document.querySelector("#productPreview2").innerHTML=p.image_url_2?`<img src="${p.image_url_2}" alt="">`:"Nenhuma imagem";document.querySelector("#productPreview3").innerHTML=p.image_url_3?`<img src="${p.image_url_3}" alt="">`:"Nenhuma imagem";document.querySelector("#productPreview4").innerHTML=p.image_url_4?`<img src="${p.image_url_4}" alt="">`:"Nenhuma imagem";document.querySelector("#productVideoPreview").innerHTML=p.video_url?`<video src="${p.video_url}" controls muted></video>`:"Nenhum vídeo";document.querySelector("#productDialog").showModal();};
...
document.querySelector("#addProductBtn").onclick=()=>{document.querySelector("#productForm").reset();document.querySelector("#productId").value="";Object.keys(mediaDeleteState).forEach(k=>mediaDeleteState[k]=false);document.querySelector("#productDialogTitle").textContent="Adicionar produto";document.querySelector("#productPreview1").textContent="Nenhuma imagem selecionada";document.querySelector("#productPreview2").textContent="Nenhuma imagem selecionada";document.querySelector("#productPreview3").textContent="Nenhuma imagem selecionada";document.querySelector("#productPreview4").textContent="Nenhuma imagem selecionada";document.querySelector("#productVideoPreview").textContent="Nenhum vídeo selecionado";populateCategorySelect(categories.find(c=>c.active)?.id);document.querySelector("#productDialog").showModal();};
function bindImagePreview(inputId,previewId){document.querySelector("#"+inputId).onchange=e=>{const f=e.target.files[0];const slot=inputId.endsWith("1")?1:inputId.endsWith("2")?2:inputId.endsWith("3")?3:4;mediaDeleteState[slot]=false;if(!f)return;const r=new FileReader();r.onload=()=>document.querySelector("#"+previewId).innerHTML=`<img src="${r.result}" alt="">`;r.readAsDataURL(f);};}
...
document.querySelector("#productVideo").onchange=e=>{const f=e.target.files[0];mediaDeleteState.video=false;if(!f)return;document.querySelector("#productVideoPreview").innerHTML=`<video src="${URL.createObjectURL(f)}" controls muted></video>`;};

document.querySelectorAll("[data-media-delete]").forEach(btn=>btn.addEventListener("click",()=>{const key=btn.dataset.mediaDelete;const inputId=key==="video"?"productVideo":`productImage${key}`;const previewId=key==="video"?"productVideoPreview":`productPreview${key}`;const input=document.querySelector("#"+inputId);const preview=document.querySelector("#"+previewId);mediaDeleteState[key]=true;if(input)input.value="";if(preview)preview.textContent=key==="video"?"Nenhum vídeo":"Nenhuma imagem";}));

document.querySelector("#productForm").onsubmit=async e=>{e.preventDefault();if(!await requireAdmin())return;try{const id=Number(document.querySelector("#productId").value),old=products.find(p=>p.id===id),file1=document.querySelector("#productImage1").files[0],file2=document.querySelector("#productImage2").files[0],file3=document.querySelector("#productImage3").files[0],file4=document.querySelector("#productImage4").files[0],video=document.querySelector("#productVideo").files[0];if(file4)await ensureFourthImageColumns();let image_url=mediaDeleteState[1]?null:(old?.image_url||null),image_path=mediaDeleteState[1]?null:(old?.image_path||null),image_url_2=mediaDeleteState[2]?null:(old?.image_url_2||null),image_path_2=mediaDeleteState[2]?null:(old?.image_path_2||null),image_url_3=mediaDeleteState[3]?null:(old?.image_url_3||null),image_path_3=mediaDeleteState[3]?null:(old?.image_path_3||null),image_url_4=mediaDeleteState[4]?null:(old?.image_url_4||null),image_path_4=mediaDeleteState[4]?null:(old?.image_path_4||null),video_url=mediaDeleteState.video?null:(old?.video_url||null),video_path=mediaDeleteState.video?null:(old?.video_path||null);if(file1){const u=await uploadMedia(file1,"products","image");image_url=u.url;image_path=u.path;}if(file2){const u=await uploadMedia(file2,"products","image");image_url_2=u.url;image_path_2=u.path;}if(file3){const u=await uploadMedia(file3,"products","image");image_url_3=u.url;image_path_3=u.path;}if(file4){const u=await uploadMedia(file4,"products","image");image_url_4=u.url;image_path_4=u.path;}if(video){const u=await uploadMedia(video,"products","video");video_url=u.url;video_path=u.path;}const category_id=Number(document.querySelector("#productCategory").value);if(!category_id)throw new Error("Selecione uma categoria.");const payload={name:document.querySelector("#productName").value.trim(),description:document.querySelector("#productDescription").value.trim(),measurements:document.querySelector("#productMeasurements").value.trim(),price:Number(document.querySelector("#productPrice").value||0),active:document.querySelector("#productActive").value==="true",category_id,image_url,image_path,image_url_2,image_path_2,image_url_3,image_path_3,image_url_4,image_path_4,video_url,video_path};if(id)await api(`/rest/v1/products?id=eq.${id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)});else{const max=products.reduce((m,p)=>Math.max(m,p.sort_order||0),0);await api("/rest/v1/products",{method:"POST",headers:{"Content-Type":"application/json","Prefer":"return=minimal"},body:JSON.stringify({...payload,sort_order:max+1})});}if(file1&&old?.image_path&&old.image_path!==image_path)await removeImage(old.image_path);if(file2&&old?.image_path_2&&old.image_path_2!==image_path_2)await removeImage(old.image_path_2);if(file3&&old?.image_path_3&&old.image_path_3!==image_path_3)await removeImage(old.image_path_3);if(file4&&old?.image_path_4&&old.image_path_4!==image_path_4)await removeImage(old.image_path_4);if(video&&old?.video_path&&old.video_path!==video_path)await removeImage(old.video_path);if(mediaDeleteState[1]&&old?.image_path)await removeImage(old.image_path);if(mediaDeleteState[2]&&old?.image_path_2)await removeImage(old.image_path_2);if(mediaDeleteState[3]&&old?.image_path_3)await removeImage(old.image_path_3);if(mediaDeleteState[4]&&old?.image_path_4)await removeImage(old.image_path_4);if(mediaDeleteState.video&&old?.video_path)await removeImage(old.video_path);document.querySelector("#productDialog").close();await loadData();}catch(e){showError(e);}};
