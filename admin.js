...
window.editIdea=id=>{const item=ideas.find(x=>x.id===id);if(!item)return;document.querySelector("#ideaForm").reset();document.querySelector("#ideaDialogTitle").textContent="Editar imagem";document.querySelector("#ideaImage").required=false;document.querySelector("#ideaImage").value="";document.querySelector("#ideaTitle").value=item.title||"";document.querySelector("#ideaDisplayTarget").value=item.display_target || (item.mobile_only ? "mobile" : "all");document.querySelector("#ideaPreview").innerHTML=item.image_url?`<img src="${item.image_url}" alt="">`:"Nenhuma imagem";document.querySelector("#ideaForm button[type=submit]").textContent="Salvar alterações";document.querySelector("#ideaDialog").showModal();};
