...
async function loadData(){
  ...
  try{
    [products,categories]=await Promise.all([
      api("/rest/v1/products?select=id,name,description,measurements,price,active,sort_order,category_id,image_url,image_path,image_url_2,image_path_2,image_url_3,image_path_3,video_url,video_path&order=sort_order.asc,created_at.asc"),
...
async function ensureThirdImageColumn(){
  try{
    await api("/rest/v1/products?select=image_url_3&limit=1");
    return true;
  }catch(e){
    throw new Error("A 3ª imagem ainda não está disponível no banco de dados. Execute no Supabase o SQL: alter table public.products add column if not exists image_url_3 text; alter table public.products add column if not exists image_path_3 text;");
  }
}
...
window.deleteProduct=async id=>{if(!confirm("Excluir este produto?"))return;try{const p=products.find(x=>x.id===id);await api(`/rest/v1/products?id=eq.${id}`,{method:"DELETE"});await removeImage(p?.image_path);await removeImage(p?.image_path_2);await removeImage(p?.image_path_3);await removeImage(p?.video_path);await loadData();}catch(e){showError(e);}};
...
document.querySelector("#addProductBtn").onclick=()=>{document.querySelector("#productForm").reset();document.querySelector("#productId").value="";document.querySelector("#productDialogTitle").textContent="Adicionar produto";document.querySelector("#productPreview1").textContent="Nenhuma imagem selecionada";document.querySelector("#productPreview2").textContent="Nenhuma imagem selecionada";document.querySelector("#productPreview3").textContent="Nenhuma imagem selecionada";document.querySelector("#productVideoPreview").textContent="Nenhum vídeo selecionado";populateCategorySelect(categories.find(c=>c.active)?.id);document.querySelector("#productDialog").showModal();};
...
bindImagePreview("productImage1","productPreview1");bindImagePreview("productImage2","productPreview2");bindImagePreview("productImage3","productPreview3");
...
document.querySelector("#productForm").onsubmit=async e=>{e.preventDefault();if(!await requireAdmin())return;try{const id=Number(document.querySelector("#productId").value),old=products.find(p=>p.id===id),file1=document.querySelector("#productImage1").files[0],file2=document.querySelector("#productImage2").files[0],file3=document.querySelector("#productImage3").files[0],video=document.querySelector("#productVideo").files[0];if(file3)await ensureThirdImageColumn();let image_url=old?.image_url||null,image_path=old?.image_path||null,image_url_2=old?.image_url_2||null,image_path_2=old?.image_path_2||null,image_url_3=old?.image_url_3||null,image_path_3=old?.image_path_3||null,video_url=old?.video_url||null,video_path=old?.video_path||null;if(file1){const u=await uploadMedia(file1,"products","image");image_url=u.url;image_path=u.path;}if(file2){const u=await uploadMedia(file2,"products","image");image_url_2=u.url;image_path_2=u.path;}if(file3){const u=await uploadMedia(file3,"products","image");image_url_3=u.url;image_path_3=u.path;}if(video){const u=await uploadMedia(video,"products","video");video_url=u.url;video_path=u.path;}const category_id=Number(document.querySelector("#productCategory").value);if(!category_id)throw new Error("Selecione uma categoria.");const payload={name:document.querySelector("#productName").value.trim(),description:document.querySelector("#productDescription").value.trim(),measurements:document.querySelector("#productMeasurements").value.trim(),price:Number(document.querySelector("#productPrice").value||0),active:document.querySelector("#productActive").value==="true",category_id,image_url,image_path,image_url_2,image_path_2,image_url_3,image_path_3,video_url,video_path};if(id)await api(`/rest/v1/products?id=eq.${id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)});else{const max=products.reduce((m,p)=>Math.max(m,p.sort_order||0),0);await api("/rest/v1/products",{method:"POST",headers:{"Content-Type":"application/json","Prefer":"return=minimal"},body:JSON.stringify({...payload,sort_order:max+1})});}if(file1&&old?.image_path&&old.image_path!==image_path)await removeImage(old.image_path);if(file2&&old?.image_path_2&&old.image_path_2!==image_path_2)await removeImage(old.image_path_2);if(file3&&old?.image_path_3&&old.image_path_3!==image_path_3)await removeImage(old.image_path_3);if(video&&old?.video_path&&old.video_path!==video_path)await removeImage(old.video_path);document.querySelector("#productDialog").close();await loadData();}catch(e){showError(e);}};
