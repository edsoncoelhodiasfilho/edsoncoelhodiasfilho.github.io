...
if (quoteForm) {
  quoteForm.addEventListener("submit", (event) => {
    ...
    // Submit directly to the hidden iframe. HTMLFormElement.submit()
    // bypasses this submit handler, so there is no navigation loop.
    quoteForm.target = "formSubmitQuoteFrame";
    HTMLFormElement.prototype.submit.call(quoteForm);

    // FormSubmit processes the POST asynchronously. After it receives the
    // submission, take the top-level page to our own thank-you page.
    window.setTimeout(() => {
...
if (emailForm) {
  emailForm.addEventListener("submit", (event) => {
    ...
    emailForm.target = "formSubmitEmailFrame";
